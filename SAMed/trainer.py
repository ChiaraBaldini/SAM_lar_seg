...
def trainer_synapse(args, model, snapshot_path, multimask_output, low_res):
    from datasets.dataset_custom import Synapse_dataset, RandomGenerator
    ...
    # # Use mixed precision to speed up the training
    ...
    iterator = tqdm(range(max_epoch), ncols=70, desc="Epoch Progress")
    for epoch_num in iterator:
        print(f"Epoch {epoch_num} started")
        for i_batch, sampled_batch in enumerate(tqdm(trainloader, ncols=70, desc=f"Epoch {epoch_num} Batches")):
        # for i_batch, sampled_batch in enumerate(trainloader):
            image_batch, label_batch = sampled_batch['image'], sampled_batch['label'] # [b, c, h, w], [b, h, w]
            # print(f"Image batch loaded: shape {image_batch.shape} ")  
            low_res_label_batch = sampled_batch['low_res_label']
            image_batch = image_batch.cuda()
            label_batch = label_batch.cuda()
            low_res_label_batch = low_res_label_batch.cuda()
            # print("Model device:", next(model.parameters()).device)
            # print("Image batch device:", image_batch.device)
            # print("Label batch device:", label_batch.device)
            # print("Low-res label batch device:", low_res_label_batch.device)
            ...
            if iter_num % 20 == 0:
                print('iteration %d : loss : %f, loss_ce: %f, loss_dice: %f' % (iter_num, loss.item(), loss_ce.item(), loss_dice.item()))
