...
def eval_mask_slice(valloader, model, criterion, opt, args):
    ...
    for batch_idx, (datapack) in enumerate(valloader):
        imgs = Variable(datapack['image'].to(dtype = torch.float32)).cuda() #, device=opt.device))
        masks = Variable(datapack['low_mask'].to(dtype = torch.float32)).cuda() #, device=opt.device))
        label = Variable(datapack['label'].to(dtype = torch.float32)).cuda() #, device=opt.device))
...
def eval_mask_slice2(valloader, model, criterion, opt, args):
    ...
    for batch_idx, (datapack) in enumerate(valloader):
        ...
        for j in range(0, b):
            ...
            # ####################### ADD for 1024 input size ##############################
            # gtn=gt
            # if isinstance(gtn, np.ndarray):
            #     gtn = torch.from_numpy(gtn)
            #     # Se gtn è [H, W], aggiungi batch dimensione
            #     if gtn.ndim == 2:
            #         gtn = gtn.unsqueeze(0)
            #     # Se serve, converti in float
            #     gtn = gtn.float()
            # gtr = F.resize(gtn, (256, 256), InterpolationMode.BILINEAR)
            # gt_i[gtr[j:j+1, :, :] == 1] = 255
            ...
            # dscs.append(dices[eval_number+j, 1])
            iou, acc, se, sp = metrics.sespiou_coefficient2(pred_i, gt_i, all=False)
    ...
    if opt.mode == "train":
        ...
        return dice_mean, iou_mean, acc_mean, se_mean, sp_mean, dices_std, iou_std, acc_std, se_std, sp_std, dscs, names
...
def eval_camus_patient(valloader, model, criterion, opt, args):
    ...
    for batch_idx, (datapack) in enumerate(valloader):
        ...
        pt = get_click_prompt(datapack, opt)
        bbox = torch.as_tensor(datapack['bbox'], dtype=torch.float32).cuda() #, device=opt.device)
    ...
    if opt.mode=="train":
        ...
        return dice_mean, iou_mean, acc_mean, se_mean, sp_mean, dices_std,iou_std, acc_std, se_std, sp_std
...
def eval_slice(valloader, model, criterion, opt, args):
    ...
    sum_time = 0
    for batch_idx, (datapack) in enumerate(valloader):
        imgs = datapack['image'].to(dtype = torch.float32).cuda() #, device=opt.device)
        masks = datapack['low_mask'].to(dtype = torch.float32).cuda(9) #, device=opt.device)
        label = datapack['label'].to(dtype = torch.float32).cuda() #, device=opt.device)
        pt = get_click_prompt(datapack, opt)
        image_filename = datapack['image_name']
...
def eval_camus_samed(valloader, model, criterion, opt, args):
    ...
    for batch_idx, (datapack) in enumerate(valloader):
        imgs = datapack['image'].to(dtype = torch.float32).cuda() #, device=opt.device)
        masks = datapack['low_mask'].to(dtype = torch.float32).cuda() #, device=opt.device)
        label = datapack['label'].to(dtype = torch.float32).cuda() #, device=opt.device)
        image_filename = datapack['image_name']
        class_id = datapack['class_id']
