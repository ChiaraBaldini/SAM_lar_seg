    ...
    for i in range(d):
        ...
        for j in range(b):
            ...
            if indices.size(0) == 0:
                ...
                random_index = torch.randint(0, h, (2,)).cuda() #.to(device = msk.device)
                new_s = msk_s
            else:
...
def get_click_prompt(datapack, opt):
    ...
    point_coords = pt
    coords_torch = torch.as_tensor(point_coords, dtype=torch.float32).cuda() #, device=opt.device)
    labels_torch = torch.as_tensor(point_labels, dtype=torch.int).cuda() #, device=opt.device)
    if len(pt.shape) == 2:
