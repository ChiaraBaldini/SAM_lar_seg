...
def main():
    ...
    parser.add_argument('--task', default='ENDOGE', help='task or dataset name')
    parser.add_argument('--vit_name', type=str, default='vit_b', help='select the vit model for the image encoder of sam')
    parser.add_argument('--sam_ckpt', type=str, default='/work/cbaldini/medSAM/code/sam_vit_b_01ec64.pth', help='Pretrained checkpoint of SAM')
    ...
    opt.load_path = "/work/cbaldini/medSAM/code/SAMUS-main/checkpoints/AutoSAMUS_GE_improved/AutoSAMUS_07300857_219_0.7309735024213018.pth" #/work/cbaldini/medSAM/code/SAMUS-main/checkpoints/GE_400ep/SAMUS__399.pth" #'/work/cbaldini/medSAM/code/SAMUS-main/checkpoints/AutoSAMUS_GE_improved/AutoSAMUS_07300857_219_0.7309735024213018.pth'
    opt.mode = "val"
    #opt.classes=2
    opt.visual = True
    #opt.eval_mode = "patient"
    args.modelname = 'AutoSAMUS'
    opt.modelname = args.modelname
    opt.eval_mode = "mask_slice"
    device = torch.device(opt.device)
    opt.test_split = "all"
    # opt.test_split = "easy_margins"
    # opt.test_split = "test"

    opt.result_path = "./result/AutoSAMUS_SEtestcontour/"
    ...
    val_dataset = ImageToImage2D(opt.data_path, opt.test_split, tf_val, img_size=args.encoder_input_size)  # return image, mask, and filename
    # val_dataset = ImageToImage2D('/work/cbaldini/medSAM/code/Seoul set', opt.test_split, tf_val, img_size=args.encoder_input_size)  # return image, mask, and filename
...
#  ========================================================================= begin to evaluate the model ============================================================================
    ...
    metrics_file = os.path.join(opt.result_path, "test_metrics.xlsx")
    # Open the file to write inference times
    with open(metrics_file, "w") as f:
        f.write("Image,DSC\n")

    if opt.mode == "train":
        dices, mean_dice,  val_losses = get_eval(valloader, model, criterion=criterion, opt=opt, args=args)
        print("mean dice:", mean_dice)
    else:
        mean_dice, mean_iou, mean_acc, mean_se, mean_sp, std_dice, std_iou, std_acc, std_se, std_sp, dscs, names = get_eval(valloader, model, criterion=criterion, opt=opt, args=args)
        with open(metrics_file, "a") as f:
            for name, dsc in zip(names, dscs):
                f.write(f"{name},{dsc:.4f}\n")
        print("dataset:" + args.task + " -----------model name: "+ args.modelname)
        print("task", args.task, "checkpoints:", opt.load_path)
        print(mean_dice[1:], mean_iou[1:], mean_acc[1:], mean_se[1:], mean_sp[1:])
        print(std_dice[1:], std_iou[1:], std_acc[1:], std_se[1:], std_sp[1:])
