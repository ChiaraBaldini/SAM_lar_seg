...
def save_masks(preds, save_path, mask_name, image_size, original_size, pad=None, boxes=None, points=None, visual_prompt=False, original_image=None):
    ...
    if visual_prompt:  # visualize the prompt
        ...
        if points is not None:
            ...
            for point, label in zip(ori_points, point_labels):
                ...
                mask[y, x] = color
                cv2.drawMarker(mask, (x, y), color='r', markerType=cv2.MARKER_CROSS, markerSize=7, thickness=2)

    os.makedirs(save_path, exist_ok=True)
    mask_path = os.path.join(save_path, f"{mask_name}")
    cv2.imwrite(mask_path, np.uint8(mask))

    # Save the original image with predictions and prompts
    if original_image is not None:
        overlay = original_image.copy()
        mask_overlay = cv2.addWeighted(overlay, 0.7, mask, 0.3, 0)
        if visual_prompt:
            if boxes is not None:
                for box in boxes:
                    cv2.rectangle(mask_overlay, (box[0], box[1]), (box[2], box[3]), (0, 255, 0), 2)
            if points is not None:
                for point, label in zip(ori_points, point_labels):
                    x, y = map(int, point)
                    color = (0, 255, 0) if label == 1 else (0, 0, 255)
                    cv2.drawMarker(mask_overlay, (x, y), color, markerType=cv2.MARKER_CROSS, markerSize=7, thickness=2)
        original_with_mask_path = os.path.join(save_path, f"original_with_{mask_name}")
        cv2.imwrite(original_with_mask_path, np.uint8(mask_overlay))
