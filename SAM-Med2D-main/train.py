...
def parse_args():
    ...
    parser.add_argument("--data_path", type=str, default="/work/cbaldini/medSAM/code/Genova set/train", help="train data path") 
    parser.add_argument("--metrics", nargs='+', default=['iou', 'dice'], help="metrics")
    parser.add_argument('--device', type=str, default='cuda')
    parser.add_argument("--lr", type=float, default=1e-4, help="learning rate")
    parser.add_argument("--resume", type=str, default=None, help="load resume") 
    parser.add_argument("--model_type", type=str, default="vit_b", help="sam model_type")
    parser.add_argument("--sam_checkpoint", type=str, default="/work/cbaldini/medSAM/code/SAM-Med2D-main/sam-med2d_b.pth", help="sam checkpoint")
...
def main(args):
    ...
    for epoch in range(0, args.epochs):
        ...
        # Salva solo il modello con la perdita migliore
        if average_loss < best_loss:
            best_loss = average_loss
            save_path = os.path.join(args.work_dir, "models", args.run_name, f"best_model_sam.pth")
            state = {'model': model.float().state_dict(), 'optimizer': optimizer}
            torch.save(state, save_path)
            loggers.info(f"Best model saved at epoch {epoch + 1} with loss {best_loss:.4f}")
