...
"""
prepare_nnunet_from_png.py
--------------------------
Convert a dataset of PNG files (images/masks) into nnU-Net v1 format.
- Supports automatic detection of Grayscale or RGB (3 channels saved as separate NIfTI files)
- Creates the folder Task{ID}_{NAME} under $nnUNet_raw_data_base
- Builds a valid dataset.json for nnU-Net
- Binarizes masks by default (0=background, 1=object)
...
Options:
  --force-rgb           Force writing 3 channels even if images are grayscale
  --allow-grayscale     Allow mixed dataset; if at least one RGB image is found → dataset is RGB
...
def save_image_nifti_channels(
    ...
    if arr.ndim == 2:
        ...
        for ch in range(3):
            img = nib.Nifti1Image(arr[..., ch], affine)
...
def save_mask_nifti(mask_png: Path, out_path: Path, dry_run: bool = False, multiclass: bool = False) -> None:
    """Save a mask as NIfTI, binarized unless --multiclass is given."""
...
def main():
    parser = argparse.ArgumentParser(description="Convert PNG (images/masks) to nnU-Net v1 format with RGB support.")
    ...
    parser.add_argument("--multiclass", action="store_true", help="Do not binarize masks (keep integer values)")
    ...
    if not args.dry_run:
        build_dataset_json(task_folder, training_entries, use_rgb=use_rgb, labels_map=labels_map)
