...
def save_image_nifti_channels(
    ...
    """Save image as a single NIfTI file with three channels."""
    affine = np.eye(4, dtype=np.float32)

    if arr.ndim == 2:
        # Grayscale image, add a singleton channel dimension
        arr = arr[:, :, None]
    elif arr.ndim == 3 and arr.shape[2] == 3:
        # RGB image, keep as is
        pass
    else:
        raise ValueError("Input array must be 2D (grayscale) or 3D with 3 channels (RGB).")

    # Save the image as a single NIfTI file
...
def save_mask_nifti(mask_png: Path, out_path: Path, image_shape: tuple, dry_run: bool = False, multiclass: bool = False) -> None:
    m = iio.imread(mask_png)
    # if m.ndim == 3:
    #     m = m[..., 0]  # Convert to grayscale if RGB

    # # Resize mask to match the image shape
    # print("Original mask shape:", m.shape)
    # print("Target image shape:", image_shape)
    # m3 = np.stack([m, m, m], axis=-1)
    # print("Stacked mask shape for resizing:", m3.shape)
    # if m3.shape[2] != image_shape[2]:
    #     print(f"Resizing mask {mask_png.name} from {m.shape[:2]} to {image_shape[:2]}")
    #     m = resize(m3, image_shape[:2], order=0, preserve_range=True, anti_aliasing=False).astype(m3.dtype)
    # else:
    #     m=m3
    m = m.astype(np.int16) if multiclass else (m > 0).astype(np.uint8)
    print("Resized mask shape:", m.shape)
    # Add a singleton channel dimension if necessary
    if m.ndim == 2:
        m = m[:, :, None]
...
def main():
    ...
    for img_path in tqdm(img_paths, desc="Convert PNG -> NIfTI"):
        ...
        img_out_path = imagesTr / f"{stem}_0000.nii.gz"
        written_imgs = save_image_nifti_channels(arr, img_out_path, dry_run=args.dry_run)
        # for img_path in written_imgs:
        nii_img = nib.load(str(img_out_path))
        print(f"Saved NIfTI file: {img_out_path}, shape: {nii_img.shape}")

        lab_out = labelsTr / f"{stem}.nii.gz"
        save_mask_nifti(mask_png, lab_out, arr.shape, dry_run=args.dry_run, multiclass=args.multiclass)
