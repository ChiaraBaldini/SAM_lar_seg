...
def aggregate_metrics(summary_files: List[Path]) -> Dict[str, Any]:
    ...
    # aggregate mean over folds for mean_dice
...
def do_preprocess(env, task_id: int) -> None:
    cmd = ["nnUNet_plan_and_preprocess", "-t", "Task501_SErgb_Seg", "--verify_dataset_integrity"]
    run(cmd, env)
...
def main():
    ...
    parser.add_argument("--task-name", type=str, required=True, help="Task name (e.g. GE_Seg)")
