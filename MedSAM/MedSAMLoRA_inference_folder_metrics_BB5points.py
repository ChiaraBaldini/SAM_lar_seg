...
@torch.no_grad()
def medsam_inference(medsam_model, img_embed, box_1024, H, W, mask_np):
    """Perform MedSAM inference with bounding box and automatically extracted point prompts"""
    ...
    # Generate foreground points (inside the bounding box)
    fg_points = [
        [np.random.randint(x_min, x_max), np.random.randint(y_min, y_max)]
        for _ in range(5)
    ]

    # Generate background points (outside the bounding box)
    bg_points = []
    while len(bg_points) < 2:
        x = np.random.randint(0, W)
        y = np.random.randint(0, H)
        if not (x_min <= x <= x_max and y_min <= y <= y_max) and not mask_np[y, x]:
            bg_points.append([x, y])

    # Combine foreground and background points
    # coords = torch.tensor([fg_points + bg_points], device=img_embed.device)  # (1, N, 2)
    # labels = torch.tensor([[1] * 5 + [0] * 2], device=img_embed.device)  # (1, N)
    coords = torch.tensor([fg_points], device=img_embed.device)  # (1, N, 2)
    labels = torch.tensor([[1] * 5], device=img_embed.device)  # (1, N)
...
sam_model = sam_model_registry["vit_b"](checkpoint="/work/cbaldini/medSAM/code/MedSAM/medsam_vit_b.pth") #/work/cbaldini/medSAM/code/sam_vit_b_01ec64.pth")
medsam_model = MedSAM(
...
# Inizializza il modello LoRA
medsam_model_lora = LoRA_Sam(medsam_model, 8).to(device)  # Sostituisci `rank` con il valore corretto

checkpoint_path = "/work/cbaldini/medSAM/code/MedSAM/work_dir/MedSAM-ViT-B-LoRA_points-20251231-1233/SAM_model_LoRA_points.pth"
checkpoint = torch.load(checkpoint_path, map_location=device)

# # Stampa tutte le chiavi disponibili
# print("Chiavi disponibili nel checkpoint:")
# for key in checkpoint.keys():
#     print(key)

# # Se il checkpoint contiene un sotto-dizionario "model", stampa le sue chiavi
# if "model" in checkpoint:
#     print("Chiavi nel sotto-dizionario 'model':")
#     for key in checkpoint["model"].keys():
#         print(key)

# Carica i parametri LoRA
medsam_model_lora.load_lora_parameters("/work/cbaldini/medSAM/code/MedSAM/work_dir/MedSAM-ViT-B-LoRA_points-20251231-1233/SAM_model_LoRA_points.pth") #/work/cbaldini/medSAM/code/MedSAM/work_dir/MedSAM-ViT-B-LoRA_points-20251231-1233/SAM_model_LoRA_points.pth")

medsam_model_lora.eval()
# Paths
...
seg_path = "/work/cbaldini/medSAM/code/MedSAM/assets/SE_test_MedSAMLoRA_5points"
os.makedirs(seg_path, exist_ok=True)

metrics_file = os.path.join(seg_path, "metrics_testSE_MedSAMLoRAfn_5points.xlsx")
metrics_list = []

# File to save inference times
...
for i in os.listdir(img_path):  # Limit to first 15 images for testing
    if not 'null' in i:
        ...
        with torch.no_grad():
            image_embedding = medsam_model_lora.sam.image_encoder(img_1024_tensor)
        ...
        for idx, (bbox_with_area, box_1024) in enumerate(zip(bboxes_with_area, boxes_1024)):
            ...
            # medsam_seg = medsam_model_lora(img_1024_tensor, point_prompt, 1024).detach().cpu().numpy()
            medsam_seg = medsam_inference(medsam_model, image_embedding, box_1024[None, :], H, W, mask_np)
            ...
            # Save inference time to file
            with open(time_file, "a") as f:
                f.write(f"{i},{inference_time:.4f}\n")

            print(f"Inference time for {i}: {inference_time:.4f} seconds")
