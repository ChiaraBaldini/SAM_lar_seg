...
for i in os.listdir(img_path):
    ...
    if not 'null' in i:
        ...
        with torch.no_grad():
            # Keep the image embedding in case you want to reuse the decoder directly.
            image_embedding = medsam_model.image_encoder(img_1024_tensor)  # (1, 256, 64, 64)

        # Use the automatic mask generator to obtain many candidate masks without explicit prompts
        # The generator expects a uint8 HxWx3 image in [0,255]
        img_for_generator = (img_1024 * 255).astype(np.uint8)
        proposals = mask_generator.generate(img_for_generator)

        # Prepare GT at 1024x1024 for matching proposals
        gt_1024 = transform.resize(mask_np.astype(np.uint8), (1024, 1024), order=0, preserve_range=True) > 0.5
        labeled_gt_1024 = label(gt_1024)
        regions_1024 = regionprops(labeled_gt_1024)

        # Visualization canvas
        fig, ax = plt.subplots(1, 2, figsize=(10, 5))
        ax[0].imshow(img_3c)
        ax[1].imshow(img_3c)
        show_mask(mask_np, ax[0], random_color=True)  # Overlay full GT mask on the left

        # For each GT connected component, find the best matching proposal (by IoU)
        for obj_idx, region in enumerate(regions_1024):
            comp_mask_1024 = (labeled_gt_1024 == region.label)
            if comp_mask_1024.sum() < 50:
            ...
            for prop in proposals:
                prop_seg = prop["segmentation"].astype(np.uint8)
                inter = np.logical_and(prop_seg, comp_mask_1024).sum()
                union = np.logical_or(prop_seg, comp_mask_1024).sum()
                iou = inter / union if union > 0 else 0.0
                if iou > best_iou:
                    best_iou = iou
                    best_proposal = prop_seg

            if best_proposal is None:
                # no proposal matched this component
                metrics_list.append({"Image": i, "Object": obj_idx, "IoU": 0.0, "DSC": 0.0})
                continue

            # Resize masks back to original image resolution for metric computation
            best_mask_orig = transform.resize(best_proposal.astype(np.uint8), (H, W), order=0, preserve_range=True) > 0.5
            comp_mask_orig = transform.resize(comp_mask_1024.astype(np.uint8), (H, W), order=0, preserve_range=True) > 0.5

            iou_final, dsc_final = compute_metrics(best_mask_orig, comp_mask_orig)
            metrics_list.append({"Image": i, "Object": obj_idx, "IoU": iou_final, "DSC": dsc_final})

            # Visualize chosen mask and bbox (map bbox from 1024->original coords)
            y0, x0, y1, x1 = region.bbox  # rowmin, colmin, rowmax, colmax in 1024
            x_min_org = int(x0 * W / 1024)
            y_min_org = int(y0 * H / 1024)
            x_max_org = int(x1 * W / 1024)
            y_max_org = int(y1 * H / 1024)

            show_mask(best_mask_orig.astype(np.uint8), ax[1])
            show_box((x_min_org, y_min_org, x_max_org, y_max_org), ax[1])

        ax[1].set_title("SAM Automatic Mask Proposals")
